...
func (db *DB) ListArtifacts(caseID string, limit, offset int) ([]models.Artifact, int, error) {
	var total int
	err := db.conn.QueryRow(`SELECT COUNT(*) FROM artifacts WHERE case_id = ?`, caseID).Scan(&total)
	if err != nil {
	...
	rows, err := db.conn.Query(
		`SELECT id, case_id, type, source, collection_method, collector_version,
		 privileges_used, sha256, compression, size_raw, size_compressed, collected_at, blob_path
		 FROM artifacts WHERE case_id = ? ORDER BY collected_at DESC LIMIT ? OFFSET ?`,
		caseID, limit, offset,
	)
	if err != nil {
...
func (db *DB) GetEventsByTimeRange(caseID string, start, end time.Time, limit, offset int) ([]models.Event, int, error) {
	...
	err := db.conn.QueryRow(
		`SELECT COUNT(*) FROM events WHERE case_id = ? AND timestamp >= ? AND timestamp <= ?`,
		caseID, startStr, endStr,
	).Scan(&total)
	if err != nil {
	...
	rows, err := db.conn.Query(
		`SELECT id, case_id, artifact_id, timestamp, source, event_id,
		 level, channel, provider, computer, message, raw_data
		 FROM events WHERE case_id = ? AND timestamp >= ? AND timestamp <= ?
		 ORDER BY timestamp ASC
		 LIMIT ? OFFSET ?`,
		caseID, startStr, endStr, limit, offset,
	)
	if err != nil {
...
func (db *DB) ListResults(caseID string, ruleID string) ([]models.YaraResult, error) {
	...
	if ruleID != "" {
		rows, err = db.conn.Query(
			`SELECT yr.id, yr.case_id, yr.rule_id, r.name, yr.artifact_id, yr.matches, yr.scanned_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.CaseID, &r.RuleID, &r.RuleName, &r.ArtifactID, &r.Matches, &scannedAt); err != nil {
...
func (db *DB) UpdateProposalStatus(id, status, result string) error {
	...
	switch status {
	case "approved":
		query = `UPDATE action_proposals SET status = ?, approved_at = ? WHERE id = ?`
		args = []interface{}{status, now, id}
	case "executed":
